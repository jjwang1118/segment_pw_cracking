...
3. **LLM Fine-tuning** — fine-tune Qwen3-4B with QLoRA to generate targeted password candidates
...
**Base Model:** `models/Qwen3-4B` (4B params; previously Llama-3.2-3B-Instruct)
**Training mode:** QLoRA — 4-bit NF4 quantization + `paged_adamw_8bit`
**Hardware:** RTX 5070 12GB, bfloat16 (bf16 disabled under QLoRA)
...
[4] util/Dataprocess.py     → datasets/processed/{tagtype}/split/train_data.jsonl
        ↓
[5] run_train.py            → checkpoints/Qwen3-4B/
```
...
- Loads Qwen3-4B in 4-bit NF4 (QLoRA) and wraps with LoRA via PEFT
...
│   └── Qwen3-4B/                 # Base LLM weights (previously Llama-3.2-3B-Instruct)
├── checkpoints/
│   └── Qwen3-4B/                 # QLoRA checkpoint directories + TensorBoard runs
├── gen/
...
lora_kind: qlora            # "lora" (bf16 LoRA) | "qlora" (4-bit NF4, for 4B+ models)

# Applied automatically when lora_kind=qlora (overrides train_config values)
qlora_train_overrides:
  bf16: false               # 4-bit model cannot use bf16 output dtype
  optim: paged_adamw_8bit   # memory-efficient optimizer for quantized training
...
train:
  ...
  train_config:
    ...
    eval_steps: 100
    save_steps: 200
    bf16: true              # overridden to false by qlora_train_overrides
...
| `src/prompt_template.py` | `prompt_convert_structure_placeholder()` (id=3, **current**): placeholder-slot prompt; `prompt_convert_token_tag()` (id=1): token+tag prompt (unused); `prompt_convert_structure_only()` (id=2): structure-only prompt (unused); `get_prompt_template(id)` |
...
**Prompt structure per training sample (id=3 `prompt_convert_structure_placeholder`, current):**
```
[System]: As a targeted password guessing model, your task is to generate likely password candidates
...
[User]: {"password structure": "(<SEG1>)(<SEG2>)(<SEG3>)",
         "segment details": {"<SEG1>": "noun",
                             "<SEG2>": "2-digit number string",
                             "<SEG3>": "1-character special symbol"}}
[Assistant]: d r a g o n 9 9 !   ← only these tokens produce loss
```

Raw tag strings (e.g. `"nn"`, `"number2"`) do not appear in the prompt; only natural-language descriptions are shown. This prevents the model from leaking tag symbols into generated passwords.
...
**QLoRA:** Model loaded in 4-bit NF4 via `BitsAndBytesConfig` (`bfloat16` compute dtype). `prepare_model_for_kbit_training()` applied before LoRA injection; `paged_adamw_8bit` reduces optimizer VRAM. Only `q_proj`, `k_proj`, `v_proj` are trained (~0.1–1% of parameters). Checkpoints store `adapter_config.json` + `adapter_weights.safetensors` (not full model weights).
...
### Inference Mode (current)

| `prompt_template_id` | What the model receives | Status |
|---|---|---|
| `3` (**current**) | system_prompt + placeholder slots `<SEG1>…<SEGN>` + natural-language descriptions per slot | Active — no raw tag strings exposed |
| `1` | system_prompt + actual token strings paired with tags + tag explanations | Unused — exposes token characters |
| `2` | system_prompt + `{"password structure": "(tag1)…", "segment details": {"position N": explanation}}` | Unused — exposes raw tag symbols |

**id=3 prompt example** (Password: `indianglaze1`, Tags: `ii|char2|char3|char3|mixed2`):
```
As a targeted password guessing model... placeholder slots... natural-language constraints...
{"password structure": "(<SEG1>)(<SEG2>)(<SEG3>)(<SEG4>)(<SEG5>)",
 "segment details": {"<SEG1>": "preposition or conjunction",
                     "<SEG2>": "2-character alphabetic string",
                     "<SEG3>": "3-character alphabetic string",
                     "<SEG4>": "3-character alphabetic string",
                     "<SEG5>": "2-character alphanumeric string"}}
```
...
contrastive_search:
  prompt_template_id: 3      # 3 = placeholder slots (current); 1 = token+tag; 2 = structure-only
...
eval:
  ...
  model_path: checkpoints/Qwen3-4B/run_1/lora_final  # optional
```
...
- **run_5 → run_2** (進行中 / baseline): checkpoints 存於 `checkpoints/Llama-3.2-3B-Instruct/run_5/`，最終輸出至 `run_2/lora_final`，prompt `id=1`, tag type `backoff`，使用 **BPE 切分** — 作為 trawling-style baseline（BPE 切分語意弱，作為對照組）（舊有 run，prompt id 已過時）
- **當前採用 prompt id=3** (`prompt_convert_structure_placeholder`): 使用 placeholder slots `<SEG1>…<SEGN>` + 自然語言描述，不暴露原始 tag 符號或 token 字串；id=1 與 id=2 目前未使用
- **Base model 改為 Qwen3-4B**（取代 Llama-3.2-3B-Instruct）；訓練模式改為 **QLoRA**（`lora_kind: qlora`）：4-bit NF4 量化 + `paged_adamw_8bit`，bf16 在 QLoRA 下自動停用
